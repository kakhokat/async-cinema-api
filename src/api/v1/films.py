...
async def films_list(
    sort: Optional[str] = Query(default="-imdb_rating", description="Example: -imdb_rating"),
    page_size: int = Query(default=50, ge=1, le=1000),
    page_number: int = Query(default=1, ge=1),
    genre: Optional[str] = Query(default=None, description="Genre UUID"),
    film_service: FilmService = Depends(get_film_service),
) -> List[FilmListItem]:
    return await film_service.list_films(sort=sort, page_number=page_number, page_size=page_size, genre=genre)
...
async def films_search(
    ...
    return await film_service.search_films(query_str=query, page_number=page_number, page_size=page_size)
